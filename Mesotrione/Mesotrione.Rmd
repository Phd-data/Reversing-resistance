...
Herbicide<-data.frame(trt=c("amitrole fb mesotrione-R", "malathion fb mesotrione-R", "PBO fb mesotrione-R", "mesotrione-R", "mesotrione-S"), y=c(28.25, 18.88, 19.31, 18.68, 32.43), se=c(1.01, 1.31, 1.32, 1.70, 1.88))
limits <- aes(ymax = y + se, ymin=y - se) 

Herbicide$trt <- factor(Herbicide$trt, levels = c("amitrole fb mesotrione-R", "malathion fb mesotrione-R", "PBO fb mesotrione-R", "mesotrione-R", "mesotrione-S")) 


ggplot(Herbicide, aes(fill=trt, x=trt, y=y)) + ggtitle("50% Mesotrione Metabolism in Waterhemp Leaves") + geom_bar(position="dodge", stat="identity", width=0.7, color="black") + geom_errorbar(limits, width=0.25, col="black") +   labs(y="Hours After Treatment", x="") + 
  ...
  theme(axis.text=element_text(size=16, color="black"), 
    ...
    panel.grid.minor = element_blank(), legend.key.size = unit(2, 'lines'),
       legend.position=c(0.38,0.78), legend.text = element_text(size = 18, colour = "black", face = "bold"), 
    axis.text.x=element_blank(), axis.ticks.x=element_blank(),
    legend.background = element_rect(colour = "white"), plot.title =  element_text(size=16, hjust=0.5, color="blue", face="italic"), legend.title=element_blank()) + scale_fill_brewer(palette="Set1") +
ggsave("ED50_poster1.tiff", units="in", width=6, height=6, dpi=500)
```
...
```{r}
library(ggplot2)
Data<-data.frame(trt=c("amitrole fb mesotrione-R", "malathion fb mesotrione-R", "piperonyl butoxide fb mesotrione-R", "mesotrione-R", "mesotrione-S"), resp=c(48.25, 24.54, 25.07, 23.69, 79.23), se=c(4.33, 0.75, 0.78, 0.67, 15.94))
limits <- aes(ymax = resp + se, ymin=resp - se) 

Data$trt <- factor(Data$trt, levels = c("amitrole fb mesotrione-R", "malathion fb mesotrione-R", "piperonyl butoxide fb mesotrione-R", "mesotrione-R", "mesotrione-S")) 

ggplot(Data, aes(fill=trt, x=trt, y=resp)) + ggtitle("80% Mesotrione Metabolism in Waterhemp Leaves") + geom_bar(position="dodge", stat="identity", color="black", width=0.7) + geom_errorbar(limits, width=0.25, col= "black") + 
  labs(y="Hours After Treatment", x="") + scale_y_continuous(limits=c(0,100)) +
  theme(axis.text=element_text(size=16, color="black"), 
        axis.title=element_text(size=18, face="bold"), 
        panel.background = element_rect(fill = "white", color = "black"), 
        panel.grid.major = element_line(color = "white"), 
    panel.grid.minor = element_blank(), legend.key.size = unit(1, 'lines'),
       legend.position=c(1.5,0.85), legend.text = element_text(size = 10, colour = "black", face = "bold"), 
    axis.text.x=element_blank(), axis.ticks.x=element_blank(),
    legend.background = element_rect(colour = "black"), plot.title =  element_text(size=16, hjust=0.5, color="blue", face="italic"), legend.title=element_blank()) + scale_fill_brewer(palette="Set1") +
