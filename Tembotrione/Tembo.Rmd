...
ggplot(Tembo, aes(x = h, y = y)) + ggtitle("Tembotrione Metabolism in Waterhemp Leaves") + #geom_point(aes(shape=TRTB, col=TRTB), size=2) +
 coord_cartesian(ylim = c(0, 100)) + labs(fill="", y="50% Tembotrione Metabolism (%)", x="Hours After Treatment") +
...
Herbicide<-data.frame(trt=c("Aminotriozole fb Tembotrione-R", "Malathion fb Tembotrione-R", "Piperonyl butoxide fb Tembotrione-R", "Tembotrione-R", "Tembotrione-S"), y=c(35.46, 87.8, 38.4, 24.3, 42.7), se=c(6.49, 9.65, 5.9, 0.6, 3.86))
...
ggplot(Herbicide, aes(fill=trt, x=trt, y=y)) + ggtitle("80% Tembotrione Metabolism in Waterhemp Leaves") + geom_bar(position="dodge", stat="identity", color="black") + geom_errorbar(limits, width=0.25, col="white") + 
  labs(y="Hours After Treatment", x="") + scale_y_continuous(limits=c(0, 110))+
  theme(axis.text=element_text(size=25, color="black"), 
        axis.title=element_text(size=30,face="bold"), 
        panel.background = element_rect(fill= "black"), 
        panel.grid.major = element_line(color = "black"), 
    panel.grid.minor = element_blank(), legend.key.size = unit(2, 'lines'),
       legend.position=c(0.71,0.87), legend.text = element_text(size = 20, colour = "black", face = "bold"), 
...
Herbicide<-data.frame(trt=c("Aminotriozole fb Tembotrione-R", "Malathion fb Tembotrione-R", "Piperonyl butoxide fb Tembotrione-R", "Tembotrione-R", "Tembotrione-S"), y=c(25.38, 43.0, 27.5, 19.2, 25.6), se=c(0.81, 2.27, 1.32, 2.55, 0.73))
limits <- aes(ymax = y + se, ymin=y - se) 
...
ggplot(Herbicide, aes(fill=trt, x=trt, y=y)) + ggtitle("50% Tembotrione Metabolism in Waterhemp Leaves") + geom_bar(position="dodge", stat="identity", color="black") + geom_errorbar(limits, width=0.25, col="white") + 
  labs(y="Hours After Treatment", x="") + scale_y_continuous(limits=c(0, 60))+
  theme(axis.text=element_text(size=25, color="black"), 
        axis.title=element_text(size=30,face="bold"), 
        panel.background = element_rect(fill= "black"), 
        panel.grid.major = element_line(color = "black"), 
